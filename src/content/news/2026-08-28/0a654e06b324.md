...
originalUrl: "https://blog.google/innovation-and-ai/technology/developers-tools/build-with-gemini-omni-1-1-flash/"
date: "2026-08-28T05:29:26.677Z"
---
...
Omni 现在提供影棚级的视频制作能力，包括场景扩展、首尾帧插值、清晰的 4K 超分辨率放大、更快的原型设计等功能。
...
我们正式推出 Gemini Omni 1.1 Flash，这是一次面向开发者的生产级更新，提供了对生成式视频更强的控制力。现在，您可以扩展场景、指定起始帧和结束帧以实现平滑过渡，并生成高分辨率的 4K 输出。立即通过 Google AI Studio 或 Gemini Enterprise Agent Platform 访问该模型，开启您的创作之旅。
...
“Gemini Omni 1.1 Flash”为开发者提供了对生成式视频项目更好的控制力。您可以将场景扩展至最长 40 秒，并保持更好的视觉一致性和叙事连贯性。通过设置起始帧和结束帧，可以创建平滑、专业的运镜和转场效果。利用 360p 预览功能，您可以在创作过程中更快地进行迭代并节省成本。最终项目可放大至 4K 分辨率，呈现出精致、专业的视觉效果。
...
今天，我们推出了 Gemini Omni 1.1 Flash，这是一套全新的创意控制工具和生成式视频功能，旨在为开发者提供支持。Gemini Omni 将现实世界的推理能力引入了生成式创作，而今天的更新使 Omni 1.1 能够通过 Google AI Studio 中的 Gemini API 投入专业生产环境。无论您是在构建生成式视频工作流、创意工具还是媒体编辑软件，这些更新都使生成式视频更具可控性、迭代速度更快，并能为实际部署提供更精良的成品。
...
场景扩展功能允许您基于现有视频，从中断处无缝继续生成画面。借助 Omni 1.1，模型现在可以分析长达 10 秒的前置上下文——这比以往仅参考最后一秒的模型有了质的飞跃。其结果是视觉一致性和叙事连贯性的提升，让您可以构建更长的故事或拓展新的创意方向。您可以以 10 秒为增量扩展视频，总时长最长可达 40 秒。
...
通过指定镜头的起始帧和结束帧，实现平滑的转场和运镜效果。Omni 1.1 可以在两个关键帧之间生成连续的视频，非常适合复杂的轨道运镜、缩放转场或无缝循环片段。
...
生成轻量级的 360p 预览，以便在进行高分辨率渲染之前，快速且低成本地迭代您的创意。