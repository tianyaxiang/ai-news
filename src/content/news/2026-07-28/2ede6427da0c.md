...
originalUrl: "https://deepmind.google/blog/introducing-gemini-3-6-flash-3-5-flash-lite-and-3-5-flash-cyber/"
date: "2026-07-27T22:38:55.967Z"
---
...
开发人员和客户在构建生产级 AI 智能体时，需要更高的 Token 使用效率、更低的延迟以及更可靠的性能。我们的 Flash 系列模型旨在平衡效率与质量，从而实现智能体工作流的规模化。在 Gemini 3.5 Flash 的基础上，我们推出了以下新模型：
...
*   **3.5 Flash-Lite：** 我们速度最快、最具成本效益的 3.5 系列模型。根据 Artificial Analysis Index，其输出速度可达每秒 350 个 Token，且在智能体工作流中的表现显著优于前几代 Flash-Lite。
...
除了今天发布的内容外，Gemini 3.5 Pro 目前正在与合作伙伴进行测试，我们计划在准备就绪后尽快向公众开放。与此同时，我们的团队已经开始专注于下一代模型的构建。我们已经启动了迄今为止最雄心勃勃的预训练项目——Gemini 4，并对目前的进展感到振奋。
...
Gemini 3.6 Flash 直接基于开发者和客户对 3.5 Flash 的反馈进行改进。3.6 Flash 不仅在编程和知识工作方面实现了质的飞跃，同时还显著提升了 Token 使用效率。例如，在 Artificial Analysis Index 中，3.6 Flash 的输出 Token 消耗量比 3.5 Flash 减少了 17%。此外，它在完成多步骤工作流时所需的推理步骤和工具调用也更少。
...
这种效率的提升还伴随着比 3.5 Flash 更低的价格。3.6 Flash 的定价为每百万输入 Token 1.50 美元，每百万输出 Token 7.50 美元，这降低了每个智能体任务的总体成本，使得构建和运行智能体更具性价比。

Even while being more efficient, 3.6 Flash sees performance gains compared to 3.5 Flash across use cases:
尽管效率更高，但 3.6 Flash 在各项用例中的性能表现相比 3.5 Flash 仍有提升：
...
*   其计算机使用能力得到了提升（如 OSWorld-Verified 测试中为 83.0% 对 78.4%）。目前，计算机使用功能已成为通过 Gemini API 和 Gemini Enterprise 提供的内置客户端工具。
...
3.6 Flash 配备了增强的前沿安全防护措施，涵盖化学、生物、放射性和核（CBRN）领域以及网络攻击滥用防范。这些防护措施使模型在抵御越狱攻击方面更加稳健。同时，模型经过训练，旨在最大限度地减少对有益用途的拒绝。
...
除了 Flash 之外，我们还发布了 Gemini 3.5 Flash-Lite，专为低延迟任务以及对开发者工作流至关重要的高吞吐量任务（如智能体搜索和文档处理）而设计。
